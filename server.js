...
async function seedRegistry(){if(!dbEnabled)return;const existing=await db.listUnits();const existingKeys=new Set(existing.map(u=>u.unitKey));const base=[["page","home","الرئيسية"],["page","admin","لوحة الإدارة"],["page","owner","لوحة المالك"],["page","client","بوابة العميل"],["page","technician","بوابة الفني"],["workflow","maintenance-order","طلب صيانة"],["feature","automatic-dispatch","التوزيع التلقائي"],["feature","payments","الدفع الإلكتروني"],["feature","audit","سجل التدقيق"]];for(const [unitType,unitKey,displayName] of base){if(existingKeys.has(unitKey))continue;const at=now();try{await db.createUnit({unitId:"unit-"+unitKey,unitType,unitKey,displayName,status:"active",version:1,ownerUid:null,permissions:{roles:["owner","admin"]},conditions:{},design:{},linking:{},timing:{},scriptRef:null,texts:{ar:displayName},data:{},notifications:{},createdAt:at,updatedAt:at})}catch(e){console.warn("[Oxygen11] Registry seed skipped:",unitKey,e.message)}}}
...
app.post("/api/registry/units",requireAuth,requirePermission("registry.write"),async(req,res)=>{try{if(!dbEnabled)return res.status(409).json({ok:false,error:"Registry requires MySQL"});const b=req.body||{},nowAt=now();if(!b.unitId||!b.unitType||!b.unitKey||!b.displayName)return res.status(400).json({ok:false,error:"unitId, unitType, unitKey and displayName are required"});const unit=await db.createUnit({...b,unitId:String(b.unitId),unitType:String(b.unitType),unitKey:String(b.unitKey),displayName:String(b.displayName),ownerUid:b.ownerUid||req.user.uid,createdAt:nowAt,updatedAt:nowAt});await db.addUnitAudit(unit.unitId,"created",req.user.uid,null,unit).catch(()=>{});res.status(201).json({ok:true,unit})}catch(e){res.status(409).json({ok:false,error:e.message})}});
...
app.patch("/api/registry/units/:id",requireAuth,requirePermission("registry.write"),async(req,res)=>{try{if(!dbEnabled)return res.status(409).json({ok:false,error:"Registry requires MySQL"});const allowed=["unitType","unitKey","displayName","status","ownerUid","permissions","conditions","design","linking","timing","scriptRef","texts","data","notifications"];const patch={};for(const k of allowed)if(req.body&&Object.prototype.hasOwnProperty.call(req.body,k))patch[k]=req.body[k];const before=await db.getUnit(String(req.params.id));if(!before)return res.status(404).json({ok:false,error:"Unit not found"});const expectedVersion=req.body?.expectedVersion==null?null:Number(req.body.expectedVersion);if(expectedVersion!=null&&expectedVersion!==Number(before.version))return res.status(409).json({ok:false,error:"Stale unit version",currentVersion:before.version});const unit=await db.updateUnit(String(req.params.id),patch,req.user.uid,String(req.body?.changeSummary||"registry_update"));await db.addUnitAudit(unit.unitId,"updated",req.user.uid,before,unit).catch(()=>{});res.json({ok:true,unit})}catch(e){res.status(400).json({ok:false,error:e.message})}});
app.get("/api/registry/units/:id/audit",requireAuth,requirePermission("registry.read"),async(req,res)=>{if(!dbEnabled)return res.status(409).json({ok:false,error:"Registry requires MySQL"});res.json({ok:true,audit:await db.getUnitAudit(String(req.params.id))})});
